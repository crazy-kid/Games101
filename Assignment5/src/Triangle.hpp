    ...
    const Vector3f& dir, float& tnear, float& u, float& v)
{
    ...
    const float s1e1 = dotProduct(s1, e1);
    tnear = 1.0 / s1e1 * dotProduct(s2, e2);
    const float b1 = 1.0 / s1e1 * dotProduct(s1, s);
    const float b2 = 1.0 / s1e1 * dotProduct(s2, dir);
    const float b0 = 1.0 - b1 - b2;
    if (tnear + DSP > 0.f && b1 + DSP > 0.f && b2 + DSP > 0.f && b0 + DSP > 0.f) {
        u = b0 * v0.x + b1 * v1.x + b2 * v2.x;
        v = b0 * v0.y + b1 * v1.y + b2 * v2.y;
        return true;
    }
    return false;
}
